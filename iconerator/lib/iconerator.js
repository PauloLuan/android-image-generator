...
iconMeta.forEach( function(meta, i, arr){
    ...
    if(meta.platform === 'iOS'){
        ...
        if(meta.type === 'Launcher Icon') 
            outputFile = outputPath + config.output.androidDir + '/res/drawable-'+meta.resolution+'/'+commander.args[0];
    ...
    gm( inputImage )
        ...
        .write( outputFile, function(err){
           ...
           else console.log("Generated %s (res: %s) icon: %s", meta.platform, (meta.resolution || ''), commander.args[0]);
        });
